...
function downloadCSV(rows: KeywordRow[]) {
  ...
  const lines = [
    header.join(','),
    ...rows.map((r) =>
      [r.keyword, r.intent, r.heat, r.competition, r.suggestion].map(esc).join(','),
    ),
  ];
  const bom = '\uFEFF';
...
export default function ToolPage() {
  ...
  const tool = getToolBySlug(__i18nSlug);

  const __i18nName = (() => {
    const fb = !tool ? '' : resolvedLocale === 'zh' ? tool.name ?? '' : (tool.nameEn ?? '') || (tool.name ?? '');
  ...
  const __i18nDesc = (() => {
    const fb = !tool ? '' : resolvedLocale === 'zh' ? tool.description ?? '' : (tool.descriptionEn ?? '') || (tool.description ?? '');
    if (resolvedLocale === 'zh' || !tool) return fb;
    const tryKey = (k: string) => {
      try {
        const v = __toolsT(k);
  ...
  const ui = (() => {
    ...
    if (l === 'zh') {
      return {
        ...
        empty: '暂无结果，试试其它核心词',
        limit: '今日免费额度',
        limitOver: '今日免费额度已用完（每天 3 次），请明天再来',
      };
    }
    if (l === 'es') {
      return {
        ...
        btn: 'Empezar minería',
        btn2: 'Minar otro lote',
        loading: 'Extrayendo palabras long-tail…',
        colKw: 'Palabra clave',
        colIntent: 'Intención',
        colHeat: 'Interés (est.)',
        colComp: 'Competencia (est.)',
        colSug: 'Sugerencia de contenido',
        ...
        limit: 'Cuota diaria gratuita',
        limitOver: 'Cuota diaria agotada (3 intentos/día). Vuelve mañana.',
      };
    }
    if (l === 'fr') {
      return {
        inputPlaceholder: 'Mot-clé de départ, ex: ai tools, télétravail…',
        btn: 'Démarrer l’exploration',
        btn2: 'Exploration suivante',
        loading: 'Extraction des mots-clés long-tail…',
        colKw: 'Mot-clé',
        colIntent: 'Intention',
        colHeat: 'Intérêt (est.)',
        colComp: 'Concurrence (est.)',
        colSug: 'Idée de contenu',
        ...
        limit: 'Quota journalier gratuit',
        limitOver: 'Quota journalier épuisé (3 essais/jour). Revenez demain.',
      };
    }
    if (l === 'hi') {
      return {
        ...
        btn: 'खनन शुरू करें',
        btn2: 'एक और बैच',
        loading: 'लॉन्ग-टेल कीवर्ड निकाले जा रहे हैं…',
        colKw: 'कीवर्ड',
        colIntent: 'इरादा',
        colHeat: 'हीट (अनुमानित)',
        colComp: 'प्रतिस्पर्धा (अनुमानित)',
        colSug: 'सामग्री सुझाव',
        ...
        limit: 'आज की मुफ्त कोटा',
        limitOver: 'आज की कोटा खत्म (3 बार/दिन)। कल फिर आना।',
      };
    }
    if (l === 'ar') {
      return {
        ...
        btn: 'ابدأ التنقيب',
        btn2: 'دفعة أخرى',
        loading: 'جاري استخراج الكلمات الطويلة…',
        colKw: 'الكلمة',
        colIntent: 'النية',
        colHeat: 'الشغف (تقدير)',
        colComp: 'المنافسة (تقدير)',
        colSug: 'اقتراح محتوى',
        exportCsv: 'تصدير CSV',
        copyAll: 'نسخ الكل',
        copied: 'تم النسخ',
        ...
        limit: 'الحد اليومي المجاني',
        limitOver: 'انتهى الحد اليومي (3 محاولات/يوم). عد غداً.',
      };
    }
    return {
      inputPlaceholder: 'Enter seed keyword, e.g. ai tools, remote work, budget planner…',
      btn: 'Start Mining',
      btn2: 'Mine Another Batch',
      loading: 'Mining long-tail keywords…',
      colKw: 'Keyword',
      colIntent: 'Intent',
      colHeat: 'Heat (est.)',
      colComp: 'Competition (est.)',
      colSug: 'Content Suggestion',
      exportCsv: 'Export CSV',
      copyAll: 'Copy All Keywords',
      copied: 'Copied',
      ...
      limit: 'Daily free quota',
      limitOver: 'Daily free quota exceeded (3 attempts/day). Please come back tomorrow.',
    };
  })();
  ...
  async function mine() {
    ...
    try {
      ...
      setError(
        (e as Error).message ||
          (resolvedLocale === 'zh' ? '挖掘失败，请稍后再试' : 'Mining failed, please try again'),
      );
    } finally {
      setLoading(false);
    }
  ...
  async function onCopyAll() {
    ...
    if (ok) {
      setCopyFlag('all');
      setTimeout(() => setCopyFlag(null), 1600);
    }
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-wrap items-center gap-1.5 text-xs sm:text-sm mb-6">
        ...
        {tool && (
          <>
            <ChevronRight className="h-3.5 w-3.5 text-gray-400 shrink-0" />
            <span className="font-medium text-gray-900 dark:text-gray-100 truncate max-w-[260px]">
              {__i18nName}
            </span>
          </>
        )}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs sm:text-sm">
                      <thead className="bg-gray-50 dark:bg-gray-800/70">
                        <tr>
                          <th className="px-3 py-3 text-left font-semibold text-gray-700 dark:text-gray-200 w-[26%]">
                            {ui.colKw}
                          </th>
                          <th className="px-3 py-3 text-left font-semibold text-gray-700 dark:text-gray-200 w-[10%]">
                            {ui.colIntent}
                          </th>
                          <th className="px-3 py-3 text-left font-semibold text-gray-700 dark:text-gray-200 w-[10%]">
                            {ui.colHeat}
                          ...
                          <th className="px-3 py-3 text-left font-semibold text-gray-700 dark:text-gray-200 w-[36%]">
                            {ui.colSug}
                          </th>
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                        {results.map((r, idx) => (
                          <tr
                            key={idx}
                            className="hover:bg-gray-50/50 dark:hover:bg-gray-800/40 align-top"
                          >
                            <td className="px-3 py-3 font-medium text-gray-900 dark:text-gray-100 break-words">
                              {r.keyword}
                            </td>
                            ...
                            <td className="px-3 py-3 text-right">
                              <button
                                onClick={() => onCopyRow(r.keyword, idx)}
