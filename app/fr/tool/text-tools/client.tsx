        ...
        <main className='lg:col-span-7'>
          <div className='card p-4 sm:p-6'>
            ...
            <div className='flex gap-1 sm:gap-2 mb-4 sm:mb-6 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-x-auto'>
              {[
                { key: 'stats', label: 'Stats du texte' },
                { key: 'case', label: 'Convertisseur de casse' },
                { key: 'dedup', label: 'Dédoublonner / Ranger' },
              ].map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4'>
              <div>
                <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>Saisir du texte</label>
                <textarea
                  value={text}
              ...
              <div>
                <div className='flex items-center justify-between mb-2'>
                  <label className='block text-sm font-medium text-gray-700 dark:text-gray-300'>
                    {activeTab === 'stats' ? 'Résultat des stats' : 'Résultat de sortie'}
                  </label>
                  {activeTab !== 'stats' && (
                    <button
                ...
                {activeTab === 'stats' && (
                  <div className='h-56 sm:h-72 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 overflow-y-auto'>
                    <div className='space-y-2.5 sm:space-y-3'>
                      <StatItem label='Caractères totaux' value={stats.chars} />
                      <StatItem label='Caractères (sans espaces)' value={stats.charsNoSpace} />
                      <StatItem label='Mots' value={stats.words} />
                      <StatItem label='Lignes' value={stats.lines} />
                      <StatItem label='Paragraphes' value={stats.paragraphs} />
                      <div className='pt-2 border-t border-gray-200 dark:border-gray-700'>
                        <p className='text-xs text-gray-500 dark:text-gray-400 mb-2'>Analyse des caractères</p>
                        <div className='space-y-1.5'>
                          <StatItem label='Caractères chinois' value={stats.chineseChars} small />
                          <StatItem label='Lettres anglaises' value={stats.englishChars} small />
                          <StatItem label='Chiffres' value={stats.digits} small />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {activeTab === 'case' && (
              <div className='mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                <ActionButton onClick={toUpperCase} disabled={!text}>MAJUSCULES</ActionButton>
                <ActionButton onClick={toLowerCase} disabled={!text}>minuscules</ActionButton>
                <ActionButton onClick={toTitleCase} disabled={!text}>Phrase</ActionButton>
                <ActionButton onClick={toSentenceCase} disabled={!text}>Phrase</ActionButton>
                <ActionButton onClick={toggleCase} disabled={!text}>iNVERSER cASSE</ActionButton>
                <button
            ...
            {activeTab === 'dedup' && (
              <div className='mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                <ActionButton onClick={removeDuplicateLines} disabled={!text}>Supprimer lignes doublons</ActionButton>
                <ActionButton onClick={removeEmptyLines} disabled={!text}>Supprimer lignes vides</ActionButton>
                <ActionButton onClick={removeExtraSpaces} disabled={!text}>Supprimer espaces en trop</ActionButton>
                <ActionButton onClick={reverseText} disabled={!text}>Inverser le texte</ActionButton>
              </div>
            )}
          </div>
        </main>
        <aside className='lg:col-span-3'>
          <div className='card p-4 sm:p-6'>
            ...
            <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              Saisir du texte puis choisissez une action à exécuter Stats du texte、Convertisseur de casse、, Dédoublonner / Ranger et plus. Traitement 100% local.
            </p>
            ...
            <ul className='space-y-2'>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                ...
                En temps réelStats du texte (caractères / mots /Lignes）
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                5  modes de Convertisseur de casse
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Supprimer lignes doublons / Supprimer lignes vides
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Analyse mixte CJK et latin
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                En un clic Copier résultat
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Local uniquement, confidentialité sûre
              </li>
            </ul>
          </div>
        </aside>
