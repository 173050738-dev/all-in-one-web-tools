...
import FigmaJsonExporter from '@/components/FigmaJsonExporter';
...
const pathLocale = (() => {
  ...
  const rawLocale = (lm && lm[1]) ? lm[1] : ''; return VALID_LOCALES.includes(rawLocale) ? rawLocale : (resolvedParams?.locale || 'fr');
})();
    ...
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8'>
        ...
        <main className='lg:col-span-7'>
          <FigmaJsonExporter locale={resolvedLocale} />
        </main>
        <aside className='lg:col-span-3'>
          <div className='card p-4 sm:p-6'>
            <h3 className='font-semibold text-gray-900 dark:text-gray-100 mb-3 sm:mb-4'>{t('guide')}</h3>
            <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              Clic droit sur la page dans Figma → Copier → Coller dans l'outil, sélectionnez les couches à exporter. Prend en charge les formats PNG et SVG, traitement entièrement local dans le navigateur.
            </p>
            ...
            <ul className='space-y-2'>
              ...
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Exportation en Lot PNG/SVG
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Analyser Structure de Couches Figma
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Sélectionner Tout/Couches Individuelles
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Aucun Compte Figma Requis
              </li>
            </ul>
          </div>
        </aside>
