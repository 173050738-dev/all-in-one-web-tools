...
const resolvedLocale = (resolvedParams?.locale && VALID_LOCALES.includes(resolvedParams.locale)) ? resolvedParams.locale : pathLocale;
  ...
  const handleDecode = () => {
    try {
      ...
      setOutput('Échec du décodage : chaîne Base64 invalide');
    }
  };
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4'>
              <div>
                <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>
                  {mode === 'encode' ? 'Saisir du texte' : 'Saisir Base64'}
                </label>
                <textarea
                  value={input}
                  onChange={(e) => setInput(e.target.value)}
                  placeholder={mode === 'encode' ? 'Saisissez le texte à encoder...' : 'Collez une chaîne Base64...'}
              ...
              <div>
                <div className='flex items-center justify-between mb-2'>
                  <label className='block text-sm font-medium text-gray-700 dark:text-gray-300'>
                    {mode === 'encode' ? 'Sortie Base64' : 'Sortie décodée'}
                  </label>
        ...
        <aside className='lg:col-span-3'>
          <div className='card p-4 sm:p-6'>
            ...
            <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              Saisissez du texte ou téléversez un fichier — encodez/décodez Base64 en un clic. Modes texte et fichier. 100% local.
            </p>
            ...
            <ul className='space-y-2'>
              ...
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                Basculement de mode en un clic
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
