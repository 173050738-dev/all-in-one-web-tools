        ...
        <main className='lg:col-span-7'>
          <div className='card p-4 sm:p-6'>
            ...
            <div className='flex gap-1 sm:gap-2 mb-4 sm:mb-6 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-x-auto'>
              {[
                { key: 'stats', label: 'إحصائيات النص' },
                { key: 'case', label: 'محول الأحرف' },
                { key: 'dedup', label: 'إزالة التكرارات / ترتيب' },
              ].map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4'>
              <div>
                <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>إدخال نص</label>
                <textarea
                  value={text}
                  onChange={(e) => setText(e.target.value)}
                  placeholder='الصق أو اكتب نصك هنا...'
              ...
              <div>
                <div className='flex items-center justify-between mb-2'>
                  <label className='block text-sm font-medium text-gray-700 dark:text-gray-300'>
                    {activeTab === 'stats' ? 'نتيجة الإحصائيات' : 'نتيجة الإخراج'}
                  </label>
                  {activeTab !== 'stats' && (
                    <button
                ...
                {activeTab === 'stats' && (
                  <div className='h-56 sm:h-72 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 overflow-y-auto'>
                    <div className='space-y-2.5 sm:space-y-3'>
                      <StatItem label='إجمالي الأحرف' value={stats.chars} />
                      <StatItem label='أحرف بدون مسافات' value={stats.charsNoSpace} />
                      <StatItem label='الكلمات' value={stats.words} />
                      <StatItem label='الأسطر' value={stats.lines} />
                      <StatItem label='الفقرات' value={stats.paragraphs} />
                      <div className='pt-2 border-t border-gray-200 dark:border-gray-700'>
                        <p className='text-xs text-gray-500 dark:text-gray-400 mb-2'>تحليل الأحرف</p>
                        <div className='space-y-1.5'>
                          <StatItem label='الأحرف الصينية' value={stats.chineseChars} small />
                          <StatItem label='الأحرف الإنجليزية' value={stats.englishChars} small />
                          <StatItem label='الأرقام' value={stats.digits} small />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {activeTab === 'case' && (
              <div className='mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                <ActionButton onClick={toUpperCase} disabled={!text}>حروف كبيرة</ActionButton>
                <ActionButton onClick={toLowerCase} disabled={!text}>حروف صغيرة</ActionButton>
                <ActionButton onClick={toTitleCase} disabled={!text}>عنوان</ActionButton>
                <ActionButton onClick={toSentenceCase} disabled={!text}>جملة</ActionButton>
                <ActionButton onClick={toggleCase} disabled={!text}>عكس الأحرف</ActionButton>
                <button
            ...
            {activeTab === 'dedup' && (
              <div className='mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                <ActionButton onClick={removeDuplicateLines} disabled={!text}>إزالة الأسطر المكررة</ActionButton>
                <ActionButton onClick={removeEmptyLines} disabled={!text}>إزالة الأسطر الفارغة</ActionButton>
                <ActionButton onClick={removeExtraSpaces} disabled={!text}>إزالة المسافات الزائدة</ActionButton>
                <ActionButton onClick={reverseText} disabled={!text}>عكس النص</ActionButton>
              </div>
            )}
          </div>
        </main>
        <aside className='lg:col-span-3'>
          <div className='card p-4 sm:p-6'>
            ...
            <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              إدخال نص ثم اختر إجراءً للتنفيذ إحصائيات النص、محول الأحرف、, إزالة التكرارات / ترتيب والمزيد. معالجة محلية 100%.
            </p>
            ...
            <ul className='space-y-2'>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                ...
                فوريإحصائيات النص (أحرف / كلمات /الأسطر）
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                5  أنماط من محول الأحرف
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                إزالة الأسطر المكررة / إزالة الأسطر الفارغة
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                تحليل مختلط CJK ولاتيني
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                بنقرة واحدة نسخ نتيجة
              </li>
              <li className='flex items-center gap-2 text-xs sm:text-sm text-gray-600 dark:text-gray-400'>
                <span className='w-1.5 h-1.5 rounded-full bg-primary-500' />
                محلي فقط، خصوصية آمنة
              </li>
            </ul>
          </div>
        </aside>
