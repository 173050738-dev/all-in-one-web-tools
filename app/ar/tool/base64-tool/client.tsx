...
const resolvedLocale = (resolvedParams?.locale && VALID_LOCALES.includes(resolvedParams.locale)) ? resolvedParams.locale : pathLocale;
  ...
  const handleDecode = () => {
    try {
      ...
      setOutput('فك فشل: سلسلة Base64 غير صالحة');
    }
  };
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4'>
              <div>
                <label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>
                  {mode === 'encode' ? 'إدخال نص' : 'إدخال Base64'}
                </label>
                <textarea
                  value={input}
                  onChange={(e) => setInput(e.target.value)}
                  placeholder={mode === 'encode' ? 'أدخل النص المطلوب ترميزه...' : 'الصق سلسلة Base64...'}
              ...
              <div>
                <div className='flex items-center justify-between mb-2'>
                  <label className='block text-sm font-medium text-gray-700 dark:text-gray-300'>
                    {mode === 'encode' ? 'الناتج Base64' : 'الناتج بعد الفك'}
                  </label>
        ...
        <aside className='lg:col-span-3'>
          <div className='card p-4 sm:p-6'>
            ...
            <p className='text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              أدخل نصاً أو ارفع ملفاً — بت مشفر/فك Base64 بنقرة واحدة. وضعي النص والملف. محلي 100%.
            </p>
