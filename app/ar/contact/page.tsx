...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent(`[تواصل Korelyy] من ${name.trim()}`);
    const body = encodeURIComponent(
      `الاسم: ${name.trim()}\nالبريد الإلكتروني: ${email.trim()}\n\n${message.trim()}\n\n— أُرسل من نموذج تواصل Korelyy Tools`
    );
  ...
  return (
    <div className='max-w-4xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-8' dir='rtl'>
      <h1 className='text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-1 sm:mb-2'>تواصل معنا</h1>
      <p className='text-[11px] sm:text-xs text-gray-500 dark:text-gray-400 mb-4 sm:mb-5'>تواصل مع فريق Korelyy</p>
      ...
      <div className='grid md:grid-cols-2 md:gap-4 sm:p-5 gap-3 sm:gap-4'>
        <div>
          <div className='bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-5 mb-6 sm:mb-8'>
            <h2 className='text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2.5 sm:mb-3'>الإعلانات</h2>
            <p className='text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              فرص إعلانية على منصتنا العالمية. وصل إلى آلاف المستخدمين من خلال التعريف بالعلامة التجارية والمحتوى الرعائي والشراكات.
            </p>
          ...
          <div className='bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-5'>
            <h2 className='text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2.5 sm:mb-3'>الدعم الفني</h2>
            <p className='text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-3 sm:mb-4'>
              تواجه مشكلة؟ تحتاج مساعدة تقنية؟ نرد على كل رسائل خلال 24 ساعة.
            </p>
            <div className='flex items-center gap-2 flex-wrap'>
              <MessageSquare className='h-4 w-4 text-primary-600 dark:text-primary-400 flex-shrink-0' />
              <a
                href={`mailto:${RECIPIENT}?subject=${encodeURIComponent('[تذكرة دعم Korelyy]')}`}
        ...
        <div className='bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-5'>
          <h2 className='text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2.5 sm:mb-3'>إرسال رسالة</h2>
          <form className='space-y-4' onSubmit={handleSubmit} noValidate>
            <div>
              <label className='block text-[11px] sm:text-xs font-medium text-gray-700 dark:text-gray-300 mb-1' htmlFor='c-name'>اسمك</label>
              <input
                id='c-name'
                type='text'
                required
            ...
            <div>
              <label className='block text-[11px] sm:text-xs font-medium text-gray-700 dark:text-gray-300 mb-1' htmlFor='c-email'>بريدك الإلكتروني</label>
              <input
                id='c-email'
                type='email'
                required
            ...
            <div>
              <label className='block text-[11px] sm:text-xs font-medium text-gray-700 dark:text-gray-300 mb-1' htmlFor='c-msg'>الرسالة</label>
              <textarea
                id='c-msg'
                rows={4}
                required
                ...
                placeholder='صف مشكلتك أو ملاحظتك...'
              />
            </div>
            <button
              type='submit'
            ...
            >
              ...
              {submitted ? 'جاري فتح التطبيق...' : 'إرسال عبر البريد'}
            </button>
            {submitted && (
              <p className='text-xs sm:text-sm text-green-600 dark:text-green-400 flex items-center gap-1.5'>
                <CheckCircle2 className='h-4 w-4' />
                سيتم فتح بريدك الإلكتروني تلقائياً. إن لم يحدث شيء، راسلنا مباشرة على {RECIPIENT}。
              </p>
            )}
            <p className='text-[10px] sm:text-xs text-gray-400 dark:text-gray-500'>
              يتم استخدام تطبيق البريد الافتراضي. لن يتم رفع أي بيانات إلى الخوادم.
            </p>
          </form>
        </div>
      </div>
