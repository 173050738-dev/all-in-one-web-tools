...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemPrompt = `你是 Korelyy Tools 工作流推荐助手。根据用户描述的任务流程，推荐 2-5 个可串联执行的工具，每步简要说明做什么和上一步的输入输出关系。以 JSON 返回 {steps:[{id, toolName, purpose, inputFromPrev, outputToNext}]}。
