...
import EcommerceImageProcessor from '@/components/EcommerceImageProcessor';
...
export default function ToolPage() {
  ...
  const __i18nName = (() => {
    ...
    return tryKey(__i18nSlug + '.name') ?? (tool.id && tool.id !== __i18nSlug ? tryKey(tool.id + '.name') : null) ?? fb;
  })();
  const __i18nDesc = (() => {
    const fb = tool?.description ?? '';
    if (resolvedLocale === 'zh' || !tool) return fb;
    const tryKey = (k: string) => { try { const v = __toolsT(k); if (v && v !== k) return v; } catch {} return null; };
    return tryKey(__i18nSlug + '.description') ?? (tool.id && tool.id !== __i18nSlug ? tryKey(tool.id + '.description') : null) ?? fb;
  })();

  const toolsT = useTranslations('tools');
  const isZhLocal = resolvedLocale === 'zh';
  const translatedToolName = tool ? isZhLocal ? tool.name : (() => { try { const t = toolsT(`${tool.id}.name`); if (t && t !== `${tool.id}.name`) return t; } catch {} return tool.name; })() : undefined;
  const translatedToolDesc = tool ? isZhLocal ? tool.description : (() => { try { const t = toolsT(`${tool.id}.description`); if (t && t !== `${tool.id}.description`) return t; } catch {} return tool.description; })() : undefined;
    ...
    <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        {tool && (() => {
          ...
          return (
            <>
              ...
              <a href={`/${resolvedLocale}?category=${cat.id}`} className='text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors truncate max-w-[180px]'>
                {sidebarT(cat.id)}
              </a>
            </>
          );
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8'>
        ...
        <main className='lg:col-span-7'>
          <EcommerceImageProcessor locale={resolvedLocale} />
        </main>
