        ...
        <main className='lg:col-span-7'>
          <div className='card p-4 sm:p-6'>
            ...
            <div className='flex gap-1 sm:gap-2 mb-4 sm:mb-6 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-x-auto'>
              {[
                { key: 'stats', label: 'Text stats' },
                { key: 'case', label: 'Case converter' },
                { key: 'dedup', label: 'Dedup / Tidy' },
              ].map((tab) => (
                <button
                  key={tab.key}
              ...
              <div>
                <div className='flex items-center justify-between mb-2'>
                  <label className='block text-sm font-medium text-gray-700 dark:text-gray-300'>
                    {activeTab === 'stats' ? 'Stats result' : 'Output result'}
                  </label>
                  {activeTab !== 'stats' && (
                    <button
                ...
                {activeTab === 'stats' && (
                  <div className='h-56 sm:h-72 p-3 sm:p-4 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 overflow-y-auto'>
                    <div className='space-y-2.5 sm:space-y-3'>
                      <StatItem label='Total chars' value={stats.chars} />
                      <StatItem label='Chars (no spaces)' value={stats.charsNoSpace} />
                      ...
                      <div className='pt-2 border-t border-gray-200 dark:border-gray-700'>
                        <p className='text-xs text-gray-500 dark:text-gray-400 mb-2'>Character analysis</p>
                        <div className='space-y-1.5'>
                          <StatItem label='Chinese chars' value={stats.chineseChars} small />
                          <StatItem label='English letters' value={stats.englishChars} small />
                          <StatItem label='Digits' value={stats.digits} small />
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {activeTab === 'case' && (
              <div className='mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                <ActionButton onClick={toUpperCase} disabled={!text}>UPPERCASE</ActionButton>
                <ActionButton onClick={toLowerCase} disabled={!text}>lowercase</ActionButton>
                <ActionButton onClick={toTitleCase} disabled={!text}>Title Case</ActionButton>
                <ActionButton onClick={toSentenceCase} disabled={!text}>Sentence case</ActionButton>
                <ActionButton onClick={toggleCase} disabled={!text}>tOGGLE cASE</ActionButton>
                <button
            ...
            {activeTab === 'dedup' && (
              <div className='mt-4 sm:mt-6 grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                <ActionButton onClick={removeDuplicateLines} disabled={!text}>Remove duplicate lines</ActionButton>
                <ActionButton onClick={removeEmptyLines} disabled={!text}>Remove blank lines</ActionButton>
                <ActionButton onClick={removeExtraSpaces} disabled={!text}>Trim extra spaces</ActionButton>
                <ActionButton onClick={reverseText} disabled={!text}>Reverse text</ActionButton>
              </div>
            )}
          </div>
        </main>
