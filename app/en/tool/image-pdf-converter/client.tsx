...
const SLUG = 'image-pdf-converter';
...
export default function ToolPage() {
  ...
  const __i18nName = (() => {
    const fb = !tool ? '' : (((tool as any).nameEn ?? '') || (tool.name ?? ''));
  ...
  const __i18nDesc = (() => {
    const fb = !tool ? '' : (((tool as any).descriptionEn ?? '') || (tool.description ?? ''));
    if (resolvedLocale === 'zh' || !tool) return fb;
    const tryKey = (k: string) => { try { const v = __toolsT(k); if (v && v !== k) return v; } catch {} return null; };
  ...
  return (
    <ToolDetailWrapper locale={resolvedLocale} slug={SLUG}>
      <ImagePdfConverter locale={resolvedLocale} />
    </ToolDetailWrapper>
  );
}
