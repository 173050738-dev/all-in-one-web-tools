...
import ClassicalChineseConverter from '@/components/ClassicalChineseConverter';
import ToolDetailWrapper from '@/components/ToolDetailWrapper';
import { getToolBySlug } from '@/data/tools';
import { usePreferencesStore } from '@/stores/preferences';

const VALID_LOCALES = ['zh', 'en', 'hi', 'fr', 'es', 'ar'];
const SLUG = 'classical-chinese-converter';
...
export default function ToolPage() {
  ...
  useEffect(() => { if (tool) addToHistory(tool.id); }, [tool, addToHistory]);

  return (
    <ToolDetailWrapper locale={resolvedLocale} slug={SLUG}>
      <ClassicalChineseConverter locale={resolvedLocale} />
    </ToolDetailWrapper>
  );
}