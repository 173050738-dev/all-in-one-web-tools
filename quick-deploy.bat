...
echo [6/8] 复制 CF Pages 静态资源 (_headers/_redirects/favicon/og-image/sitemap)...
if not exist out (
...
REM ---------- 7. 静态写入每个工具页的 SEO Meta + OG/Twitter + canonical ----------
echo [7/8] 写入每个工具页独立的 SEO Meta (title/OG/Twitter/canonical) ...
node scripts\rewrite-meta.cjs
if errorlevel 1 (
  echo.
  echo [ERROR] Meta 静态写入失败！
  exit /b 4
)

REM ---------- 8. 上传到 Cloudflare Pages ----------
echo [8/8] 上传到 Cloudflare Pages 项目: %PAGES_PROJECT%
