...
export default function InstallToHomeButton() {
  ...
  useEffect(() => {
    const onCloseAll = (e: Event) => {
      ...
      if (ev.detail?.except === 'install') return;
      setMenuOpen(false);
      setActiveGuide(null);
    };
  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={() => {
          if (!menuOpen) {
            window.dispatchEvent(new CustomEvent('close-all-overlay-panels', { detail: { except: 'install' } }));
          }
      ...
      {justInstalled && (
        <div className="fixed left-1/2 -translate-x-1/2 top-16 sm:absolute sm:right-0 sm:top-full sm:left-auto sm:translate-x-0 w-[min(92vw,280px)] sm:w-72 z-50 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-200 text-xs px-3 py-2.5 shadow-2xl animate-in fade-in slide-in-from-top-2 ring-1 ring-emerald-500/20" style={{ maxWidth: 'calc(100vw - 24px)' }}>
      ...
      {menuOpen && !activeGuide && (
        <>
          <div className="fixed inset-0 z-40 sm:hidden bg-black/40 backdrop-blur-[2px] animate-in fade-in" onClick={() => setMenuOpen(false)} />
          <div className="fixed inset-0 z-40 hidden sm:block" onClick={() => setMenuOpen(false)} />
          <div
            ref={menuRef}
            className="
              ...
              overflow-hidden
              animate-in fade-in slide-in-from-bottom-8 sm:slide-in-from-top-2
            "
            style={{ maxHeight: '88dvh', WebkitTapHighlightColor: 'transparent', display: 'flex', flexDirection: 'column' }}
          >
            <div className="px-4 py-3.5 border-b border-gray-100 dark:border-gray-700 flex items-start gap-3 bg-gradient-to-b from-indigo-50/60 to-transparent dark:from-indigo-950/30 relative flex-shrink-0">
              <div className="sm:hidden absolute left-1/2 -translate-x-1/2 -top-2 w-10 h-1 rounded-full bg-gray-300/80 dark:bg-gray-600/70" aria-hidden="true" />
            ...
            <div className="px-2.5 py-2.5 flex-1 min-h-0 overflow-y-auto space-y-1.5 pb-[max(10px,env(safe-area-inset-bottom))]">
              {/* ════════════ C 优化：一键安装视觉升级 ════════════ */}
              <div
      ...
      {activeGuide && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/45 backdrop-blur-[2px] animate-in fade-in">
          <div className="w-full sm:max-w-lg sm:mx-auto bg-white dark:bg-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden animate-in slide-in-from-bottom-8 sm:slide-in-from-bottom-0 duration-200" style={{ maxHeight: '90dvh', display: 'flex', flexDirection: 'column' }}>
            <div className="flex items-center justify-between px-4 sm:px-5 py-3.5 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-b from-gray-50/70 to-transparent dark:from-gray-900/40 flex-shrink-0">
            ...
            <div className="p-4 sm:p-5 space-y-3.5 flex-1 min-h-0 overflow-y-auto pb-[max(10px,env(safe-area-inset-bottom))]">
