...
export default function ToolDetailWrapper({ locale, slug, children }: ToolDetailWrapperProps) {
  ...
  const toolName = tool
    ? isZh
      ...
      : (() => {
          try {
            const translated = toolsT(`${tool.id}.name`);
            if (translated && translated !== `${tool.id}.name`) return translated;
          } catch {
            /* fallthrough */
          }
          return tool.name;
  ...
  const toolDescription = tool
    ? isZh
      ...
      : (() => {
          try {
            const translated = toolsT(`${tool.id}.description`);
            if (translated && translated !== `${tool.id}.description`) return translated;
          } catch {
            /* fallthrough */
          }
