  ...
  return (
    <div className='relative w-full' ref={dropdownRef}>
      <div className='relative'>
        <Search className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${isAiMode ? 'text-purple-400' : 'text-gray-400'}`} />
        <input
          ref={inputRef}
          type='text'
          ...
          className={`w-full pl-10 pr-20 py-2 rounded-lg border bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:ring-2 transition-shadow ${
            isAiMode
        ...
        <div className='absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1'>
          ...
          >
            ...
            <span className='hidden sm:inline'>{isAiMode ? (locale === 'zh' ? '返回' : 'Back') : 'AI'}</span>
          </button>
        </div>
      </div>

      {isOpen && (
        <div className={`absolute left-0 right-0 top-full mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200 ${isMobile ? 'fixed left-3 right-3 top-16 mt-0' : 'w-full'}`}>
          {isAiMode ? (
            <div className='py-2 max-h-[60vh] sm:max-h-[50vh] overflow-y-auto'>
              {isAiLoading ? (
                  ...
                  <div className='flex flex-wrap gap-1.5 justify-center'>
                    {(locale === 'zh' ? ['做PPT用什么工具', '图片压缩加水印', '写论文找资料', '开发工具推荐'] : ['Tools for PPT design', 'Image compress + watermark', 'Research paper tools', 'Developer tools']).map((q) => (
                      <button
                        ...
                        className='px-2.5 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full hover:bg-purple-100 dark:hover:bg-purple-900/30 hover:text-purple-700 dark:hover:text-purple-300 transition-colors'
                      >
                        {q}
                      </button>
                    ))}
                ...
                <>
                  <div className='px-4 py-3 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 border-b border-purple-100 dark:border-purple-900/30'>
                    ...
                    <p className='text-sm text-gray-500 dark:text-gray-400'>
                      {locale === 'zh' ? 'AI暂时没有推荐，以下是相关搜索结果：' : 'No AI recommendations, here are search results:'}
                    </p>
                  </div>
                  <div className='py-2'>
                    {results.length > 0 ? (
            ...
            <div className='p-3 sm:p-4 max-h-[60vh] sm:max-h-[50vh] overflow-y-auto'>
                  ...
                  <div className='flex flex-wrap gap-1.5'>
                    {searchHistory.map((q) => (
                      <button
                        ...
                        className='px-2.5 py-1 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors truncate max-w-[150px]'
                      >
                        {q}
                      </button>
                    ))}
