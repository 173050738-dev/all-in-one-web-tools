          ...
          <div className='mb-3 sm:mb-4 -mx-3 sm:-mx-4 px-3 sm:px-4'>
            <div className='flex items-center gap-2 overflow-x-auto pb-1 category-scroll' style={{ scrollbarWidth: 'thin' }}>
              <button
                ...
                className={`whitespace-nowrap px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-colors ${selectedCategory === 'all' ? 'bg-[#0F5759] text-white hover:bg-[#0C4849]' : 'bg-gray-100 dark:bg-gray-800 text-[#466B6C] dark:text-gray-300'}`}
              >
                {tSidebar('all')}
              </button>
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`whitespace-nowrap px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-colors ${selectedCategory === cat.id ? 'bg-[#0F5759] text-white hover:bg-[#0C4849]' : 'bg-gray-100 dark:bg-gray-800 text-[#466B6C] dark:text-gray-300'}`}
                  >
                    {cat.name}
                  </button>
                );
          ...
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 sm:mb-6'>
            ...
            <div className='flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1'>
              <a
                ...
                className='flex items-center gap-1 whitespace-nowrap px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium bg-gradient-to-r from-[#0F5759] to-[#34A89C] text-white hover:from-[#0C4849] hover:to-[#2E968B] transition-all shadow-md shadow-[#0F5759]/15 hover:shadow-lg hover:shadow-[#0F5759]/20 active:scale-95'
              >
              ...
              <button
                ...
                className={`whitespace-nowrap px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-colors ${sortBy === 'newest' ? 'bg-[#0F5759] text-white hover:bg-[#0C4849]' : 'bg-primary-50 dark:bg-gray-800 text-[#466B6C] dark:text-gray-300 hover:bg-primary-100 dark:hover:bg-gray-700'}`}
              >
                {t('sort-newest')}
              </button>
              <button
                onClick={() => setSortBy('popular')}
                className={`whitespace-nowrap px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-colors ${sortBy === 'popular' ? 'bg-[#0F5759] text-white hover:bg-[#0C4849]' : 'bg-primary-50 dark:bg-gray-800 text-[#466B6C] dark:text-gray-300 hover:bg-primary-100 dark:hover:bg-gray-700'}`}
              >
                {t('sort-popular')}
              </button>
              <button
                onClick={() => setSortBy('free')}
                className={`whitespace-nowrap px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm transition-colors ${sortBy === 'free' ? 'bg-[#0F5759] text-white hover:bg-[#0C4849]' : 'bg-primary-50 dark:bg-gray-800 text-[#466B6C] dark:text-gray-300 hover:bg-primary-100 dark:hover:bg-gray-700'}`}
              >
                {t('sort-free')}
              </button>
            </div>
