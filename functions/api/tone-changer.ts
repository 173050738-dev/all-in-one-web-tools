...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const { text, tone, locale = 'zh' } = payload;
...
请用 ${langLabel} 输出。保持原文的核心意思不变。`;
    ...
    if (!result.rewritten) {
      return errJson('No rewritten text generated', 500);
    }
    ...
    return okJson({ error: 'Internal server error', detail: msg, hasKey: !!env?.ARK_API_KEY, hasUrl: !!env?.ARK_API_URL }, 500);
  }
};