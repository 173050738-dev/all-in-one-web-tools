...
// Cookie key（Cloudflare Pages _worker.js 通过此 Cookie 记忆用户手动选择，避免 Accept-Language 覆盖）
...
export function setLocaleCookie(locale: SupportedLocale): void {
  ...
  try {
    ...
    const secure = typeof window !== 'undefined' && window.location.protocol === 'https:'
      ...
      : '';
    document.cookie =
      `${LOCALE_COOKIE_KEY}=${locale}; Path=/; Max-Age=${maxAge}; SameSite=Lax${secure}`;
  } catch {
    // 忽略错误
  }
}
...
/**
 * 从 localStorage 或 Cookie 读取用户手动选择的语言
 * 优先级：localStorage > Cookie（localStorage 是用户在本站手动切的权威来源；Cookie 用于传给 CF Worker 做边缘重定向）
 */
...
/**
 * 保存用户手动选择的语言 → localStorage + Cookie 双写
 * Cookie 的作用：Cloudflare Pages _worker.js 在边缘读到此 Cookie，就不再按 Accept-Language 重定向
 */
